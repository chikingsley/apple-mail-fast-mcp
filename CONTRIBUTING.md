...
```bash
git clone https://github.com/chikingsley/apple-mail-fast-mcp.git
...
1. **Before you start coding,** open an issue (or comment on an existing one) describing what you plan to fix or build. This lets us flag duplicate or in-flight work and saves you from rebases or wasted effort.
...
1. Implement backend (`mail_connector.py`) and frontend (`server.py`) together

1. Run checks: `make check-all`

1. **If your PR touches IMAP or AppleScript code paths** — `imap_connector.py`, `mail_connector.py` AppleScript bodies/wrappers, or tools gated by `_elicit_confirmation` — also run `make test-e2e` before pushing (requires `MAIL_TEST_MODE=true`; the happy-path dispatch tests are mocked and need no account, but the full suite needs a test Mail.app account). **CI does not run e2e** — they need Mail.app — so a stale e2e failure on `main` is only caught by someone running this locally.
...
1. Update `docs/reference/TOOLS.md` if you added/changed a tool
1. PR description references the issue (`Closes #N`)
