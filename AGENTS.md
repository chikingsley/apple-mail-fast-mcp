...
- Use `uv` for Python versions, dependencies, locking, execution, auditing, and builds. The project targets Python 3.14 and keeps runtime and packaged maintenance commands in a `src` layout.
...
- Tests belong in exactly two categories: `tests/regressions` for a documented issue or bug and `tests/live` for real Apple Mail, IMAP, SMTP, or MCP behavior. Do not add coverage-driven or routine mocked tests. `uv run apple-mail-project check-test-policy` enforces this boundary.
